    ...
    <div>
        <div class="container mx-auto px-4 py-8">
            <div class="flex items-center flex-col justify-center mb-8">
                <img :src="user.photoURL" alt="Profile Picture" class="w-48 h-48 rounded-full object-cover">
                ...
                <div v-if="showModal"
                    ...
                    <div class="relative bg-white rounded-lg p-8 m-8">
                        ...
                        <div class="flex flex-col items-center">
                            <img :src="previewImage || user.photoURL" class="w-48 h-48 rounded-full object-cover" />
                            <input type="file" ref="photoUpload" accept="image/png, image/jpeg" class="mb-4"
                                @change="handleFileChange">
                            <button @click.prevent="handleUpdate" class="px-4 py-2 bg-blue-500 disabled:bg-blue-200 text-white rounded-md"
                                :disabled="!previewImage">Update</button>
                        </div>
                    </div>
                </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-1 gap-4 self-center">
                <div>
                    ...
                    <div class="bg-white rounded-lg p-4 shadow">
                        ...
                        <p><span class="font-semibold">Authentication Method:</span> {{ user.providerData[0].providerId }}
                        </p>
                        <p><span class="font-semibold">Anonymous:</span> {{ user.isAnonymous ? 'Yes' : 'No' }}</p>
                        <p><span class="font-semibold">UID:</span> {{ user.uid }}</p>
                        <p><span class="font-semibold">Created At:</span> {{ formatDate(user.metadata.createdAt) }}</p>
                        <p><span class="font-semibold">Last Login At:</span> {{ formatDate(user.metadata.lastLoginAt) }}</p>
                    </div>
                </div>
